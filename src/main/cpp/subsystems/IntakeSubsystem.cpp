...
IntakeSubsystem::IntakeSubsystem() 
    :   m_pivotMotor(IntakeConstants::kPivotMotorID, rev::CANSparkMax::MotorType::kBrushless),
        m_pivotController(m_pivotMotor.GetPIDController()),
        m_pivotEncoder(m_pivotMotor.GetAbsoluteEncoder(rev::SparkAbsoluteEncoder::Type::kDutyCycle)),
        m_intakeMotor(IntakeConstants::kIntakeMotorID, rev::CANSparkMax::MotorType::kBrushless),
        ...
        m_intakeTarget(IntakeState::kStopped) {
            ...
            m_pivotEncoder.SetPositionConversionFactor(IntakeConstants::kPositionConversion);
...
void IntakeSubsystem::ControlLoop() {
    ...
    m_pivotController.SetReference(ToOutput(m_pivotTarget), rev::CANSparkMax::ControlType::kPosition);
}
...
void IntakeSubsystem::UpdatePivotState() {
    ...
    // Check ground
    if (hb::InRange(position, IntakeConstants::Positions::kGround.value(), IntakeConstants::Positions::kTollerance.value())) {
        m_pivotCurrent = PivotState::kGround;
        return;
    }

    // Check stow
    if (hb::InRange(position, IntakeConstants::Positions::kStow.value(), IntakeConstants::Positions::kTollerance.value())) {
