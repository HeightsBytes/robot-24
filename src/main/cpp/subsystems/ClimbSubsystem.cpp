...
m_motorLeft(ClimbConstants::kMotorLeftID, rev::CANSparkMax::MotorType::kBrushless),
m_motorRight(ClimbConstants::kMotorRightID, rev::CANSparkMax::MotorType::kBrushless),
m_encoderLeft(m_motorLeft.GetEncoder(rev::SparkRelativeEncoder::Type::kHallSensor)),
m_encoderRight(m_motorRight.GetEncoder(rev::SparkRelativeEncoder::Type::kHallSensor)),
...
m_rightTarget(ClimbConstants::Positions::kStow) {
        ...
        m_encoderLeft.SetPositionConversionFactor(ClimbConstants::kPositionConversion);
        m_encoderRight.SetPositionConversionFactor(ClimbConstants::kPositionConversion);
        ...
        m_controllerLeft.SetOutputRange(ClimbConstants::kMinPower, ClimbConstants::kMaxPower);
        m_controllerRight.SetOutputRange(ClimbConstants::kMinPower, ClimbConstants::kMaxPower);
...
frc2::CommandPtr ClimbSubsystem::SetSyncBehaviorCMD(ClimbSubsystem::Behavior behavior) {
...
void ClimbSubsystem::InitSendable(wpi::SendableBuilder& builder) {
  ...
  builder.AddDoubleProperty("Left Height", LAMBDA(GetLeftHeight().value()), nullptr);
  builder.AddDoubleProperty("Right Height", LAMBDA(GetRightHeight().value()), nullptr);

  builder.AddDoubleProperty("Left Target", LAMBDA(m_leftTarget.value()), nullptr);
  builder.AddDoubleProperty("Right Target", LAMBDA(m_rightTarget.value()), nullptr);
